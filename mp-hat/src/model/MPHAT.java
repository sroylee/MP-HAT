...
public class MPHAT {
	...
	private double getLikelihood() {
		...
		for (int u = 0; u < dataset.nUsers; u++) {
					...
					for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
						...
						if (currUser.platforms[p] == 1) {
							...
							if (followingPlatform == p) {
								...
								HupAvp = HupAvp * lamda;
								linkRelationshipLikelihood += Math.log(1-Math.exp(-HupAvp)) - Math.log(Math.exp(-HupAvp)+1);
							}
						}
					}
				}
			...
			linkLikelihood += linkRelationshipLikelihood + linkAuthorityLikelihood + linkHubLikelihood + postPlatformLikelihood;
			...
			for (int s = 0; s < currUser.nPosts; s++) {
				if (currUser.postBatches[s] == batch) {
					...
					postPlatformLikelihood += Math.log(Math.exp(currUser.topicalPlatformPreference[currPost.topic][currPost.platform])/denominator);
			...
			for (int k = 0; k < nTopics; k++) {
				postXLikelihood += ((kappa-1)*Math.log(currUser.topicalInterests[k])) - (currUser.topicalInterests[k]/theta) ;
				for (int p=0; p < Configure.NUM_OF_PLATFORM; p++){
		...
		postLikelihood = postWordLikelihood + postPlatformLikelihood+ postTopicLikelihood + postXLikelihood + postEtaLikelihood + postTauLikelihood;
